...
% Set how Code will appear in the document.
\lstset{ 
...
%Numerical solution of a first-order Hamilton--Jacobi of pathogen cohorts} %Numerical solution of a first-order Hamilton--Jacobi equation for dynamic equilibrium of pathogen cohorts}
...
% a resident and a mutant population.
...
% The Cauchy problem for the resulting first-order
...
% Interpretations of the numerical simulation obtained from this package are also explored.
% Interpretations of the obtained numerical simulation results are also
% provided.
...
% The nutrient flux uptaken by cohort~$ i $ is determined by a function $ f_i = f_i(M_1, M_2) $ and allocated 
between two
...
%also that spores are produced with a constant yield $ \delta > 0 $ as compared to mycelial growth.
...
\end{equation}
Moreover, the following parameter values are useful for the numerical simulations and arise from the analysis of \cite{YegorovGrognardMailleretHalkett2017}:
\begin{equation}
\arraycolsep=1.3pt
...
\end{equation}
is an invariant set in the state space (if a state trajectory starts in $ G $, it cannot leave $ G $).
...
%(we mention the latter inequality to include the case for when the infimum $ u_1 $ is not reached).
...
For points~$ (t, M_1, M_2) $ at which the value function is differentiable, one can obtain the saddle feedback control strategies from the relations
$$
\begin{aligned}
...
\section{Code used for the Numerical ROC-HJ}

As is the case with many differential equations, it is challenging to solve this equation, which means we need to use a numerical solver to do this task. We used ROC-HJ, which is written in C++ and allows for a fast and easy transcription of the functions above into machine code. 
Most of the preconfigured files and procedures of ROC-HJ allow for a quick transcription of a Hamilton-Jacobi equation.


\cite{ROCHJ2019} provides further details of the type of assignments and other variables that need additional configuration in order for ROC-HJ to find a solution. To use ROC-HJ, we must assign a few variables to describe the kind of problem. Fig. \ref{code:defaults} contains some of the variables which describe the problem.
%In our case, we mainly edited our data file and assigned; an example of a few of the default configurations that must be set can be found one in fig. \ref{code:defaults}.
\begin{figure}
    \inputminted[
        frame=lines,
        framesep=2mm,
        ...
        firstline=17, 
        lastline=20]{c}{data.h}
\caption{A few configuration variables needed to solve the equation using ROC-HJ}
\label{code:defaults}
\end{figure}

With these as well as many more options that we had to set to correctly assign the type of problem that we needed for the system to function precisely, we also needed to specify both a cost function (fig. \ref{code:cost}) and a dynamic equation (fig. \ref{code:dynamic}) to produce the data that we need
...
\begin{figure}
    ...
    \inputminted[frame=lines,
        ...
        %linenos,
        firstline=156, 
        lastline=176]{c}{data.h}
    \caption{Function in C++ written for ROC-HJ describing the distributed cost equation.}
    \label{code:cost}
\end{figure}
...
\begin{figure}
    ...
    \inputminted[frame=lines,
        ...
        %linenos,
        firstline=133, 
        lastline=154]{c}{data.h}
    \caption{A C++ function for use by ROC-HJ to describe the required dynamic equation. 
    %This function is written in C++ for ROC-HJ describing the dynamic equation.
    }
    \label{code:dynamic}
\end{figure}
...
%but these have been left out to save space, and more information about the file can be found
...
\begin{figure}
\center{ \includegraphics[width = 0.48 \textwidth]{Figure_4a.pdf}
\includegraphics[width = 0.48 \textwidth]{Figure_4b_1.pdf} \\
\includegraphics[width = 0.48 \textwidth]{Figure_4c_1.pdf}
\includegraphics[width = 0.48 \textwidth]{Figure_4d_1.pdf}
}
\bf \caption{\it Finite-difference approximation of the value function~$ V $ at several time instants.}
\label{Fig_4}
\end{figure}

\begin{figure}
\center{ \includegraphics[width = 0.48 \textwidth]{Figure_5a_1.pdf}
\includegraphics[width = 0.48 \textwidth]{Figure_5a_2.pdf} \\
\includegraphics[width = 0.48 \textwidth]{Figure_5b_1.pdf}
\includegraphics[width = 0.48 \textwidth]{Figure_5b_2.pdf} \\
\includegraphics[width = 0.48 \textwidth]{Figure_5c_1.pdf}
...
White and gray colors show the regions of the control values $ 0 $ and $ 1 ${\rm ,} respectively. Also illustrated are the fields of
the corresponding forward-time velocities.}
\label{Fig_5}
\end{figure}
...
increase of $ \tau $, the feedback control portrait approaches a stationary form. 
Fig.~\ref{Fig_7} is a three-dimensional portrait of the four control switching surfaces.
% A three-dimensional picture of the four control
% switching surfaces is given in Fig.~\ref{Fig_7}. 
For $ \tau \geqslant \tau^{**} $, the point~$ \left( m^{**}, m^{**} \right) $ attracts
forward-time trajectories and determines a steady-state equilibrium regime for both cohorts, while one can interpret the four surfaces as turnpikes.

\begin{figure}
\center{ \includegraphics[width = 0.98 \textwidth]{Figure_7.pdf} }
...
%they can be approached through evolution.
...
I.~Yegorov, F.~Grognard, L.~Mailleret, F.~Halkett, P.~Bernhard. A dynamic game approach to uninvadable strategies for biotrophic
pathogens. {\it Dynamic Games and Applications} 2020; {\bf 10}(1): 257--296.

\bibitem{ROCHJ2019}
Bokanowski,~O., Desilles,~A., Zidani,~H., and Zhao,~J.
User's guide for the ROC-HJ solver: Finite Differences and Semi-Lagrangian methods. January~21,~2019. Version 2.5.4.
...
[Research Report] RR--7314, INRIA, 2010. URL: \url{https://hal.inria.fr/inria-00491489v2}

\bibitem{DercoleRinaldi2008}
Dercole,~F. and Rinaldi,~S.
{\it Analysis of Evolutionary Processes{\rm :} The Adaptive Dynamics Approach and Its Applications}.
...
Subbotin,~A.\,I.
{\it Generalized Solutions of First-Order PDEs{\rm :} The Dynamical Optimization Perspective}.
...
Osher,~S. and Shu,~C.-W. High order essentially non-oscillatory schemes for Hamilton--Jacobi equations.
...
{\it Numerical Recipes{\rm :} The Art of Scientific Computing}. Cambridge University Press: New~York, 2007.
