...
%todo: add motivation
In this paper, we study a zero-sum feedback game %todo: add a definition in appendix and cite
for a system of 
nonlinear ordinary differential equations %todo: add details to appendix and cite or find resource to cite to
describing the one-seasonal dynamics of %todo: reword, make basic
two biotrophic fungal cohorts within a shared host plant. %todo 
The first fungus can be understood as the resident, while the second is the mutant.  The 
invasion functional %todo
takes the form of the difference between the 
two marginal fitness criteria %todo
and represents the cost in defining the 
differential game's value. %todo
A general dynamic game approach of
\cite{YegorovGrognardMailleretHalkettBernhard2019}
is explored and discussed. 
The Cauchy problem %todo: give more information, define in the appendix and cite
for the first-order Hamilton--Jacobi partial differential equation (HJ PDE) created by this
...
To determine the nutrient flux %todo: define and cite
...
Let $ g = g(M) $ be the rate of mycelial decay for both cohorts. Assume the constant yield of spores, $ \delta > 0 $, as compared to mycelial growth. %todo: describe this more simply
...
state variables. %todo: define and cite
...
\begin{equation}
%define and describe the Jacobi function, maybe add something to appendix
...
saddle control strategies %todo: define and cite, give example in appendix
...
\inf_{u_1(\cdot)} \:  \sup_{u_2(\cdot)} %todo: add more information and describe what infimum and supremum, maybe describe before or cite in the bottom appendix
\:\: \mbox{or} \:\:
...
the nutrient fluxes %todo: cite what nutrient fluxes are
and decay rates %todo: describe decay rates
as
\begin{equation}
...
\end{equation}
Moreover, the following parameter values are useful for the numerical
...
understood as 
carrying capacity estimates. %todo: define carrying capacities at least, reference from appendix


\section{Uninvadable and evolutionarily stable strategies}
%todo: define uninvadable give example
%todo: define evolutionrily stable strategies give example
%todo: add motivation

This section describes our motivation for the 
game-theoretic %todo: define and cite
statement
{\rm \cref{7,8,9}}.
...
nonsmooth %todo: define and provide example
and understood in a generalized
(viscosity %todo: provide more explanation
or 
minimax %todo define minimax and cite
...
in our problem is 
Lipschitz continuous %todo: define in appendix and cite
in $ [0, T] \times G $. 
Rademacher's theorem, %todo: describe in appendix and cite
is differentiable almost everywhere in $ (0, T) \times G $, except
for possibly a 
subset of Lebesgue measure zero. %todo: define and cite

For points~$ (t, M_1, M_2) $ at which the value function is differentiable,
one can obtain the saddle feedback control strategies from the relations
$$
\begin{aligned}
...
\end{equation}
These are resource allocation strategies that enable for equilibrium
coexistence of the two pathogen cohorts. %todo: describe in more detail

\section{Code used for the Numerical ROC-HJ}
%todo: add motivation
%todo: add details on how and where to access
...
Monotone finite difference methods (including the 
classical Lax--Friedrichs scheme %todo: 
...
\cdot \left( M^{**}, M^{**} \right) $. With the further increase of $ \tau $,
the feedback control portrait approaches a stationary form. Fig.~\ref{Fig_7}
is a three-dimensional portrait of the four control switching surfaces. For
$ \tau \geqslant \tau^{**} $, the point~$ \left( m^{**}, m^{**} \right) $
...
\Commonsection{Common terms}
\begin{itemize}
    \item \textbf{Zero-sum feedback game}: 
    \item \textbf{Pathogen}: this is an infectious organism, this paper mostly discusses fungal invaders.
    \item \textbf{Resident pathogen}: this is the original organism and has not been mutated.
    \item \textbf{Mutant pathogen}: this is a strain of the same species of organism as the resident species, but has been mutated, through physical separation or natural mutation.
    \item \textbf{Mycelia}: the ``roots'' of a fungus, made of extremely fine branches called hyphae. 
    \item \textbf{Cohort}: cohort typically means a person or thing that is typically on ``your side''. In this paper it typically refers to a pathogen that belongs to the same species, but differs slightly and is not of the same organism.
    \item \textbf{Biotrophic}: describes a parasite attached to plant. Describing a plant parasite.
    \item \textbf{Nutrient flux}: the amount of nutrients consumed by the pathogen.
\end{itemize}
