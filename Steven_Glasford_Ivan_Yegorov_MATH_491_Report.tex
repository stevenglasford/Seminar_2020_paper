...
\bf \caption{\it The regions that are shaded white, the fungus enters spore production, while in the gray it enters mycelia production at the particular time instance. Also, the field corresponds to the its velocity of production.
...
feedback control approaches a stationary form and there is less competition for resources between the fungi. Fig.~\ref{Fig_7}
is a three-dimensional graph of the 
four control switching surfaces (the direction for the fungi to progress and avoid invasion). 
For $ \tau \geqslant \tau^{**} $, the point~$ \left( m^{**}, m^{**} \right) $
attracts forward-time trajectories and determines a steady-state equilibrium
strategy for both cohorts, while one can interpret the four surfaces as necessary actions of the cohorts to avoid invasion.
...
In this report we focus solely on resource allocation equilibrium for the one-seasonal dynamics of two biotrophic fungal cohorts within a shared host plant. 
Though not mentioned in this paper, it is also relevant to investigate 
long-seasonal dynamics where the cohorts compete with each other on a much longer term, other relevant situations occur when the pathogens gain the ability to evolve and the amount and spore producing capacity, and when efficiencies of the pathogens begin to differ. Also interesting are situations where there are two separate and different species of fungi.
...
\begin{enumerate}
    \item\label{term:zerosum} \textbf{Zero-sum feedback game}: a game in which one of the players wants the game to reach an open target (wants to take over, invade), while the want to avoid this target forever (prevent a take over, defend)\cite{zerosumgame}.
    ...
    \item\label{term:statevariables} \textbf{State Variables}: these are variables that set the stage of the equation or system and define the initial circumstances of the problem. These include items like lesion density, or the rate of decay in the system.
...
\begin{enumerate}
   % \item\label{math:jacobi} \textbf{Jacobi}
    ...
    \item\label{math:supremum} \textbf{Supremum}: the subset of the least element that is greater than or equal to all elements of S if such an element exists \cite{infimumsupremum}.
    \item\label{math:Argmaxmin} \textbf{Arg max/Arg min}: set of a function in which the values are maximized (Arg max) or minimized (Arg min).
    ...
    \item\label{math:maximinminimax} \textbf{Maximin / Minimax}: the largest in a series of minima (maximin), or the smallest in a series of maxima (minimax).
    %\item\label{math:valuefunction} \textbf{Value function}:
    \item\label{math:cauchyproblem} \textbf{Cauchy problem}: a problem that asks for the solution of a partial differential equation that satisfies certain conditions that are given on a hypersurface.
    ...
    \item\label{math:lebesgue} \textbf{Lebesgue measure}: a standard method of assigning a measure to a subset. Can be thought of as an a unit of measure.
